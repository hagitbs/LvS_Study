...
def plot_final_analysis(df_doc, doc_id, secondary_col, filename):
    ...
    ax1.bar(x, df_doc["expected"], width=0.8, color='#D3D3D3',  
            label='Expected', alpha=0.8, zorder=2)
    # Observed bar (Inner, Blue)
    ax1.bar(x, df_doc["observed"], width=0.4, color="#8bacc3",  
...
for yr in years:
    ...
    plot_final_analysis(subset, yr, 'LvS', os.path.join(out_dir, f"LvS_No_arr_{str(yr)}.png"))
