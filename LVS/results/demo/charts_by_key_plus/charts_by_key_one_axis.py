...
def plot_key_bars(df_key: pd.DataFrame, key_value: str, out_dir: str):
    ...
    gap = df_key["gap_val"].astype(float).to_numpy()

    fig, ax = plt.subplots(figsize=(18, 7))

    # Wide grey bars (expected)
    ax.bar(x, expected, width=0.80,
           color="#9ca3af", alpha=0.9,
           label="expected", zorder=1)

    # Narrow blue bars (observed)
    ax.bar(x, observed, width=0.35,
           color="#2563eb", alpha=0.95,
    ...
    # 🔹 Unified Y range
    ax.set_ylim(
        *symmetric_ylim(np.r_[expected, observed, lvs, gap])
    )
...
def main():
    ...
    out_dir = f"results/{dataset}/charts_by_key_one_axis"
