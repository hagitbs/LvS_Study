...
def plot_final_analysis(df_doc, doc_id, secondary_col, filename):
    ...
    ax1.bar(x, df_doc["expected"], width=bar_width_expected, color='#D3D3D3',  
            edgecolor='white', linewidth=0.1, label='Expected', alpha=0.8, zorder=2)
    
    ax1.bar(x, df_doc["observed"], width=bar_width_observed, color="#8bacc3",  
            edgecolor='navy', linewidth=0.1, label='Observed', alpha=1.0, zorder=3)
