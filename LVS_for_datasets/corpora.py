...
class Corpus:
    def __init__(self, doc_cat: pd.Series | pd.CIndex, element_cat: pd.Series):
        self.doc_cat = pd.Categorical(doc_cat, ordered=True).dtype
        self.element_cat = pd.Categorical(element_cat, ordered=True).dtype

    def update_dates(self, date):
        self.doc_cat = pd.CategoricalDtype(
            self.doc_cat.categories[~self.doc_cat.categories.isin([date])],
            ordered=True,
        )

    def code_to_cat(self, code: str, what="doc") -> int:
        return getattr(self, f"{what}_cat").categories[code]

    def pivot(self, data: pd.DataFrame) -> Matrix:
        d = data["doc"].astype(self.doc_cat)
    ...
    #         d = {
    #             "doc": self.doc_cat.categories.astype(str).to_list(),
    ...
    #         data = json.load(f)
    #     return Corpus(pd.to_string(data["doc"]), pd.Series(data["elements"]))
